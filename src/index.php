...
const OUTPUT = './../build/grid.build.css';
const ROOT = __DIR__;
...
$resultCSS = file_get_contents(PARTS . 'reset.css');
$resultCSS .= file_get_contents(PARTS . 'options.css');
$resultCSS .= $css;
//$resultCSS .= file_get_contents(PARTS . 'grid-columns.css');
$resultCSS .= file_get_contents(PARTS . 'styles.css');

if (MINIFICATION) {
    $resultCSS = str_replace(array("\r\n", "\r", "\n", "\t", " "), '', $resultCSS);
}

if (file_put_contents(OUTPUT, $resultCSS)) {
    print_r("Build source into " . OUTPUT . "\n");
} else {
    print_r("Failed creating file: " . OUTPUT . "\n");
}
